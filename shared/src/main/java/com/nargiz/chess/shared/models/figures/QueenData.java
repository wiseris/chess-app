    ...
    public ActionResult validateAction(GameData gameData, ActionCommand action) {
        ...
        for(FigureData figure: gameData.getFigures().values()) {
            ...
            if (    figure.getPosition().isDiagonalBetween(getPosition(), targetPosition)
                  || (figure.getPosition().isSameRow(getPosition()) && targetPosition.isSameRow(getPosition()) && figure.getPosition().isColumnBetween(getPosition(), targetPosition))
                  || (figure.getPosition().isSameColumn(getPosition()) && targetPosition.isSameColumn(getPosition()) && figure.getPosition().isRowBetween(getPosition(), targetPosition))
            ) {
                result.addError(new ValidationError("Impossible move"));
                return result;
            }
